...
let evenNumbers = mixedNumbers.filter((x,y,z)=>x%2===0)
...
//Code Here
let postTaxPrices = prices.map((x,y,z)=>x*1.07)
...
//Code Here
let totalPopulation = populations.reduce((t,x,y,z)=>t += x)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((x,y,z)=> x.CP > 200)
...
let ordersTotal = orders.map((x)=>{x.price * x.tax})
ordersTotal.reduce((t,x)=>t += x)
